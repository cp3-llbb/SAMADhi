def DbStore(credentials='~/.samadhi'):
    ...
    cred = loadCredentials(credentials=credentials)
    db_connection_string = "mysql://{login}:{password}@{hostname}/{database}".format_map(cred)
...
class Dataset(Storm):
  ...
  def __init__(self, name, datatype):
    ...
    self.name = name
    if datatype=="mc" or datatype=="data":
